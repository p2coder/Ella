...
FIRST_DECISION_TEMPLATE = PromptTemplate(
    ...
    instruction=(
        ...
        "complex task that benefits from child-agent orchestration, call the "
        "visible workflow Tool when available. workflow is a Tool execution, "
        "not a persistent plan or graph. "
        "For a simple task, call a needed visible Tool or answer directly. "
        "Always return the best meaningful intent available. Missing execution "
        "details such as location, quantity, preference, or file path do not "
        "make intent null: keep the overall goal and call the visible user-question "
        "capability when only the user can provide those details. If the overall "
        "purpose is genuinely unclear, use a weak but honest goal describing the "
        "needed clarification and ask the user. Do not use keyword-specific goal "
        "templates. Correct any decision_repair error in WorkSpace."
    ),
    output_contract=(
        "Return one strict JSON object with intent and action. intent is always "
        "{\"goal\":\"<one concrete outcome>\",\"constraints\":[],"
